...
2. **All markers in text MUST pass through `unescapeMarkers()`.** React's `renderToStaticMarkup` HTML-escapes `"`, `'`, `>`, `<`, `&` in text nodes, which breaks block markers like `{{#eq foo "bar"}}`. Plano B (double unescape, before and after juice) is the official pipeline — NOT an emergency fallback. See `validation/test-pipeline.tsx` and CONTEXT.md.

3. **The fork `react-email-bridge-ui` follows its own rules.** Before editing files inside `packages/react-email-bridge-ui/`, read PATCHES.md for what each patch does and VENDORING.md for the rebase process. Biome is configured to ignore that directory so upstream rebases stay clean.

4. **DECISIONS.md is frozen.** 16 design decisions from v0.1 are locked there as historical record. New decisions go to `docs/adr/` (created on demand by `/grill-with-docs`), not appended to DECISIONS.md. If a new decision supersedes one in DECISIONS.md, cite the superseded D-number from the ADR.
...
- `STATUS.md` — current state of v0.1 (snapshot)
- `DECISIONS.md` — 16 design decisions from v0.1 (frozen historical record)
- `ROADMAP.md` — v0.2+ pending work
- `CONTEXT.md` — shared vocabulary (READ FIRST on new session)
- `PATCHES.md` — fork patches in `react-email-bridge-ui/`
- `VENDORING.md` — rebase process for the fork
- `NOTICE.md` — attribution of runtime deps + reference repos
- `docs/adr/` — architectural decisions from v0.2 onwards
- `docs/agents/` — files generated by `/setup-matt-pocock-skills`
